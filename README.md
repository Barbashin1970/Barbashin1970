...
## 🎿 SKICODING — рабочий навык, готовый SKILL для ИИ
...
На опыте выяснилось что модель пишет код, через неделю пять несовместимых архитектур и никто не помнит почему. Либо ты написал 4000+ строк md без единой правки в коде. Несколько проектов я искал, как держать темп без потери прозрачности. Получилось вот что: чередовать текстовые шаги (`.md`) и кодовые (`commit`), как лыжные следы — левая, правая, левая, правая. Каждый шаг оставляет проверяемый артефакт, который сверяется с предыдущим. Назвал это **SKICODING**.

Побочный эффект - одна фича закрывается за 2-4 часа против 1-2 недель в моей первой IT-команде 2022 года. Грубо ×50 по темпу. Это про то, что между PM, dev и QA нет очередей — я один играю все три роли на каждой итерации и ни одну не пропускаю. Документ открытый. Забирай, адаптируй под свою связку «человек + модель», и если станет полезно — расскажи, что у тебя получилось.
...
| **POLINOM-JAVA** | Верификатор алгоритмических рисков для Java-кода + JMH-бенчмарки производительности | [![Open](https://img.shields.io/badge/▶_Открыть-B45309?style=for-the-badge)](https://polinom-java.up.railway.app/) |
...
| **SIGMA** | Голосовой ИИ-аналитик мэрии по открытым данным городов РФ| [![Open](https://img.shields.io/badge/▶_Открыть-475569?style=for-the-badge)](https://nsk-opendata-bot.up.railway.app/) |
